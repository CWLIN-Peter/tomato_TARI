...
tb <- readxl::read_xlsx("2019_spring.xlsx",
...
# 2. filtering----------
#Filter out unwanted accessions and unify different batches of the same accession
...
unique(tb_filtered$T2.品系代碼) %>% sort() #268 accessions
...
out <- apply(tb_filtered[,c(8:11)], MARGIN = 1, FUN = any.zero)
out <- out > 0 | is.na(out)
...
#writexl::write_xlsx(tb_out, path = "有疑問的資料_108hot.xlsx")
...
#咖啡    紅  紅黃  紅橙  粉紅  淡黃    黃  黃紅    綠  褐紅    橘  橙紅 
#50  34924    10   367  2048  1353  2064    19  1433     7   414   109 
...
#咖啡    紅  紅黃  紅橙  粉紅  淡黃    黃  黃紅    綠  褐紅    橘  橙紅 
#32  26969    10   292  1603  1040  1547    19   879     7   347   107
tb_filtered$`C1.整體顏色(原果色)`[is.na(tb_filtered$lyco_1)] %>% table()
#咖啡   紅 紅橙 粉紅 淡黃   黃   綠   橘 橙紅 
#18  7955   75  445  313  517  554   67    2 
...
#0.00   14.86   18.59   18.01   21.87   30.00    7955
...
#Min.   1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#0.0049 17.0511 20.8101 20.0727 24.2782 29.9762     445 
...
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#0.0451  7.1448  9.0525  8.5666 10.4468 15.7341     517 
...
#Min.   1st Qu.   Median     Mean  3rd Qu.     Max.     NA's 
#0.02357  1.72599  2.50747  2.58853  3.29546 12.16478      313 
...
#Min.  1st Qu.  Median    Mean   3rd Qu.    Max.    NA's 
#0.0024  0.7400  1.6481  3.3274  3.2919 28.4063     554
...
#Min. 1st Qu.  Median    Mean 3rd Qu.    Max.    NA's 
#0.1367  9.2731 11.3516 10.3268 12.6268 14.7089      67 
tb_filtered$lyco_1[orange & withcol & nolyco] <- mean(tb_filtered$lyco_1[orange & withcol], na.rm = T)

#Fix ineffective lycopene values for "紅橙" and "橙紅" fruits with median
summary(tb_filtered$lyco_1[orred & withcol])
#Min.    1st Qu.   Median     Mean  3rd Qu.     Max.     NA's 
#0.02813  7.46116 10.61309 10.49887 13.48472 24.53262       77 
tb_filtered$lyco_1[orred & withcol & nolyco] <- mean(tb_filtered$lyco_1[orred & withcol], na.rm = T)
...
sum(WVratio >= 1.2, na.rm = T) #2404
sum(c(WVratio < 1.2 & WVratio > 1.1), na.rm = T) #697
sum(c(WVratio >= 0.9 & WVratio <= 1.1), na.rm = T) #14255
sum(c(WVratio < 0.9 & WVratio > 0.8), na.rm = T) #13079
sum(WVratio <= 0.8, na.rm = T) #12457
summary(WVratio) #Median = 0.8696
med <- 0.8696

sum(WVratio >= med+0.2, na.rm = T) #3535
sum(c(WVratio < med+0.2 & WVratio > med+0.1), na.rm = T) #5121
sum(c(WVratio >= med-0.1 & WVratio <= med+0.1), na.rm = T) #24915
sum(c(WVratio < med-0.1 & WVratio > med-0.2), na.rm = T) #7256
sum(WVratio <= med-0.2, na.rm = T) #2065
...
avg_2019 <- readxl::read_xlsx(path = "../phenotype data.xlsx",
                              sheet = "2018-2019")
avg_2019 <- avg_2019[,c(1,7:9,11,10)]
...
c(avg$fruit.weight - avg_2019$`fruit_weight(g)_2019hot`) %>% mean() #1.141615
cor(avg$fruit.weight, avg_2019$`fruit_weight(g)_2019hot`) #0.9623574
#numbers of accessions that have fruit weight calculated from raw data identical with the provided fruit weight
c(avg$fruit.weight == avg_2019$`fruit_weight(g)_2019hot`) %>% sum() #89
c(round(avg$fruit.weight,digits = 1) == avg_2019$`fruit_weight(g)_2019hot`) %>% sum() #13

c(avg$fruit.width == avg_2019$`fruit_width(cm)_2019hot`) %>% sum() #139
c(avg$fruit.length == avg_2019$`fruit_length(cm)_2019hot`) %>% sum() #143
unique(tb_out$T2.品系代碼) %>% length() #131 accessions 
...
plot(yield, avg_2019$`yield(kg/ha)_2019hot`)
cor(yield, avg_2019$`yield(kg/ha)_2019hot`) #0.9867771
sum(yield == avg_2019$`yield(kg/ha)_2019hot`) #14
yield_comp <- data.frame(accession = comp_phe,
                         yield_from_RAW = yield,
                         yield_2019hot_TARI = avg_2019$`yield(kg/ha)_2019hot`,
                         diff = yield - avg_2019$`yield(kg/ha)_2019hot`)
yield_comp$diff_percent <- yield_comp$diff/yield_comp$yield_2019hot_TARI*100

#writexl::write_xlsx(yield_comp, path = "產量比較_108 熱季.xlsx")

avg$yield <- yield_comp$yield_from_RAW

avg_fromRAW_2019hot <- avg

save(avg_fromRAW_2019hot, file = "./avg_fromRAW_2019hot.RData")
